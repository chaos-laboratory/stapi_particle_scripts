...
String dsUomDefs[nDs] = {"number of particles per million particles",
                        "http://www.qudt.org/qudt/owl/1.0.0/unit/Instances.html#DegreeCelsius", 
                        "percentage of humidity in the air"}; //visit "http://www.qudt.org/qudt/owl/1.0.0/unit/Instances.html" to get the definition
...
String appName = "stapi_1_scd30";
String dsIdString = String::format(
    "{ \"%s\" : \"%d\", \"%s\" : \"%d\", \"%s\" : \"%d\" }", 
    "dsId_co2",  dsId_co2, "dsId_airTemp", dsId_airTemp, "dsId_relHumid", dsId_relHumid);
...
void loop() {
    if (thingCnt == -1) {
        ...
        dsId_co2 = dsIds[0];
        dsId_airTemp = dsIds[1];
        dsId_relHumid = dsIds[2];
    ...
    // THIS IS WHERE YOU INPUT THE SCRIPT TO GET DATA FROM THE SENSOR AND POST DATA TO THE DATABASE 
        ...
        carbonDioxide = scd30.getCO2();
        temperature = scd30.getTemperature();
        rHumidity = scd30.getHumidity();
        ...
        timeNow.c_str(), temperature, dsId_airTemp);
        ...
        "{ \"time\" : \"%s\", \"result\": \"%f\", \"dsId\": \"%d\"}", 
        timeNow.c_str(), rHumidity, dsId_relHumid);
